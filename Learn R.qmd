...
So, this function could use some further development---we might want to build in some error messages and warnings, depending on how (and by whom) it will be used---but it is a good first effort. 
...
Suppose the researcher would like a summary table with a count of patients and the average result from each lab. This analysis is now straightforward using dplyr functions:
...
Missing observations are a common feature of real data sets. 
...
- `paste0()`: Concatenates objects and "pastes" them together into a single text string without spaces (that's what the `0` means).
- `paste()`: The same as `paste0()` except that spaces are added.
