...
However, we can fix this error if we *name* the arguments. Naming overrides position in function calls.
...
Previously we saw how to subset *vectors*. To return the first observation in the `rivers` dataset, for example, we would type `rivers[1]` in the console, placing that observation's index within square brackets. Or, to return all the river lengths greater than 1000 miles we would type `rivers[rivers > 1000]`. The condition specified within the square brackets evaluates as a logical vector. R uses the location of the TRUEs in that vector to index the observations.
...
[^34]: The pipe operator was introduced in the magrittr package in 2014.  Recently, a  pipe operator has been added to base R: `|>`. We will use the magrittr pipe in these lessons. For basic uses the two pipes function identically.
...
The return value here is a data frame with a row for each level of the grouping variable.^[This data frame is technically a "tibble," a special type of data frame used by the tidyverse.  The main difference is in the way a tibble prints to the screen---it will show the first few rows and only the columns that fit on one screen.] The two columns correspond to the grouping variable and the defined summary statistic. (In the latter case the name of the column comes from the arbitrary name we used within the `summarize()` function.) We can easily add information to this table by defining more summary statistics, with one per line, and lines separated by commas:
...
In RStudio go ahead and click the Knit button in the menu immediately above the document.  This will compile the document and open it (if you left the file type as `html_document`) as a web page.  This should give you a good sense of how R Markdown is translated into HTML.

## Coding Style

Analyzing data in a notebook like R Markdown will help make your work reproducible---improving collaboration and productivity---but only if your code is readable. The example code in these lessons has been written to model readability. I hope you have started to internalize that style. Let's review some guidelines for R coding style.

Here is code that is hard to read and understand:
...
- Put functions on separate lines connected by pipes. This way we can see easily see the operations on the data in sequence.^[I usually don't use a pipe for just one function:  `filter(mtcars, wt > 25)` is fine. But when two or more functions are nested it is best to unpack them.]
...
These are opinionated guidelines; style is *style*, after all---each to their own.  For example, in the last code chunk I added the object assignment on the last line. That is because I think it is more *readable*, especially for people new to R, for the flow of the piped code to go top-to-bottom, right-to-left.  You may choose to do the assignment in the first line going right to left.  That would be fine.  Just remember that readability should be one of your main goals in programming.

## Lists

Back to programming.  The data structures we've discussed so far include vectors and data frames.
**Lists** are also very useful and common. Recall:
...
Lists are more flexible than either vectors or data frames in that they can be used to group together any mix of R structures and objects.  For example, a list could contain a vector, a data frame, or even another list.

Use the `list()` function to create a list, with the elements separated by commas. 

```{r}
random_objects <- list(mtcars, rivers, list(1:8, letters), c("curly", "moe", "larry"))

```

These elements can be named while creating the list:
...
We can investigate list structure with the familiar `str()` function:
...
1. A data frame, `old_cars`.
2. A numeric vector, `river_lengths`.
3. A list, `numbers-letters`, consisting in a numeric vector,`numbers`, and a character vector, `letters`.
4. A character vector, `three_stooges.`
...
Equivalently, we can index the list with double square brackets. Since `three_stooges` is the fourth element, we would type:^[We could use the *name* of the list element also: `random_objects[["three_stooges"]]` is exactly equivalent to `random_objects[[4]]` because `three_stooges` is the 4th list element.  Note that to index with a name requires using quotation marks.]
...
We can also stack dollar signs and then subset the resulting vector with square bracket notation:

```{r}
random_objects$numbers_letters$numbers[1]
```

Is there a reason to choose one approach---dollar sign notation rather than square bracket notation with numeric indexes---over the other when subsetting? Not really. They both work.  Keep in mind, though, that numeric indexing can run into problems if, say, the list is updated with another element.  That could throw off the numeric indexes and cause the code to break. 


## Linear Regression (optional)

Lists are used in R for storing, among other things, **model objects**---the information returned after fitting a model.  Let's use **linear regression** as an example.  The purpose here is not to explain regression in much detail but to continue working with lists by exploring a common use case.
...
- $\hat{y_i}$ is the predicted value of the target.
...
model
```
The regression model equation can be written:  $\widehat{mpg} = 37.285 - 5.344 \space \text{x} \space wt$.^[In statistics the hat notation being used here, $\widehat{mpg}$, means "predicted". ] With this equation we can predict `mpg` for any value of `wt`. Those predictions lie on the regression line defined by the intercept and slope:
...
Of course, there are other aspects of this model, beyond the intercept and slope, that may be of interest. Additional information is available in the model object and---the most common source---the model summary. 

```{r}
summary(model)
```

The `summary()` function prints this information but in the background creates a list object that we can look at with `str()`:
...
Wow.  This is indeed a complicated list object!  The intercept and slope appear here (under `$ coefficients`)  along with a lot of other information, including the model fit statistics  R-squared and Adjusted R-squared. Let's take a look at `coefficients`.  

```{r}
summary(model)$coefficients
```

This is technically a **matrix**---essentially a two dimensional vector---with row and column names. (What looks like the first column is actually the row names.) To extract the intercept and slope numbers we would use square bracket notation, as in a data frame:
...
The output from the summary of a model object is designed to be looked at and interpreted.  However, it is sometimes useful to write code to extract certain elements---for example, to include a report---rather than manually typing the numbers. Suppose I want to report model R-squared in the text without having to transcribe it.  In RMarkdown I could write **inline code** to extract the number from the model and insert it into my sentence, like this:  "The model R-squared is `r summary(model)$r.squared %>% round(2)`."  To produce that number I used a single backtick to open the inline code, followed by an `r`, and then a concluding backtick after the code: "The model R-squared is 'r  summary(model)$r.squared %>% round(2)'." This is especially helpful if you are writing a repeating report that includes numbers that will change.
...
Easy. Use the `data.frame()` function to name and define columns. The process is similar to creating a list, except that data frames are more restrictive.   In particular:

- Each column is a vector, meaning: the observations need to have the same data type.
- Each column must have the same number of observations. This is because data frames are "square": each column has the same number of rows, and each row has the same number of columns.
...
Here both columns have 3 observations. This code produces an error because the columns have different lengths.
...
Most modeling algorithms require tabular data, which is why data frames are so common in analytics.  But you may sometimes need to store non-tabular data.  In that case use a list.
...
There are a couple of helpful functions for efficiently creating data frame columns:  `seq()` and `rep()`.
...
- `print_items <-`: Saves the function as an object in the environment. This will be the name of the function.
- `function (x) { }`: Initializes the function with an argument, `x`.  The arguments to a function go within the parentheses and the body of the function, containing the code to be executed, goes within the curly braces. In this case we want `x` to be returned.
- `x`: Executes the code by returning `x`. 

This function, once defined, can be called like any other:

```{r}
print_items(x = 1:5)
```

Using the function entails supplying an input for the argument.  This is the value of a function: it performs routine tasks but with varying inputs.  So, if you find yourself writing the same code repeatedly to solve similar tasks, consider writing a function!

Admittedly, this is not a very useful function, since it performs a fixed action, always printing the numbers 1 through 5.  It would be more useful if the user could specify the 
